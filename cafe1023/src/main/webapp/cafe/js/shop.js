document.addEventListener('DOMContentLoaded', function() {
	...
	function updateSltftText(crushOption, weightOption, productName) {
		if(productName == '3'){
			elements.sltftElement.textContent = `[10월 원두콩] Gallery Alice \n ${crushOption} / ${weightOption}`;
		} else if(productName == '5') {
			elements.sltftElement.textContent = `단풍 블렌드 \n ${crushOption} / ${weightOption}`;
		} else if(productName == '6'){
			elements.sltftElement.textContent = `강릉 블렌드\n ${crushOption} / ${weightOption}`;
		} else if(productName == '7'){
			elements.sltftElement.textContent = `강릉 블렌드 드립백 세트 (10개입) \n ${crushOption} / ${weightOption}`;
		}
